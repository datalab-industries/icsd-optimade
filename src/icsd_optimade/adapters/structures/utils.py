...
class UncertainFloat(BaseModel):
    ...
    def __init__(self, value: float | str, uncertainty: float = 0.0) -> None:
        if isinstance(value, str):
            value, uncertainty = self._cast_real_str_to_uncertain_float(value)

        super().__init__(value=value, uncertainty=uncertainty)

    def _cast_real_str_to_uncertain_float(self, value: str) -> tuple[float, float]:
        if "(" in value:
    ...
    def _cast_real_to_uncertain_float(
        self, other: Union[Real, "UncertainFloat"]
    ) -> "UncertainFloat":
        if isinstance(other, Real):
            other = UncertainFloat(float(other), 0.0)
        return other
    ...
    def __add__(self, other: Union[Real, "UncertainFloat"]):
        ...
        x = ufloat(self.value, self.uncertainty) + ufloat(
    ...
    def __radd__(self, other: Union[Real, "UncertainFloat"]):
        other = self._cast_real_to_uncertain_float(other)
        x = ufloat(other.value, other.uncertainty) + ufloat(
    ...
    def __lt__(self, other: Union[Real, "UncertainFloat"]):
        other = self._cast_real_to_uncertain_float(other)
        return ufloat(self.value, self.uncertainty) < ufloat(
            other.value, other.uncertainty
        )
    ...
    def __eq__(self, other: object):
        ...
        elif isinstance(other, UncertainFloat):
            return ufloat(self.value, self.uncertainty) == ufloat(
                other.value, other.uncertainty
            )
